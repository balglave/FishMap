...
#' @importFrom fmesher fm_rcdt_2d
#' @importFrom sf st_as_sf st_intersects st_join st_coordinates
#' @importFrom methods as
#' @importFrom terra ext ncell as.points as.polygons values rast
#' 
...
#' @param grid_projection character A character string of projection arguments for `sf` and `terra` objects
...
fm_build_domain_mesh_spde <- function(
  ...
  grid <- rast(ext = grid_limit, res = resol, crs = grid_projection)
  
  # Create spatialpolygon based on raster
  gridpolygon <- as.polygons(grid, dissolve = FALSE)
  ...
  raster_to_point <- as.points(grid)
  raster_to_point$layer <- 1:nrow(raster_to_point)
  terra::crs(raster_to_point) <- grid_projection
  datapoint_sf <- st_as_sf(raster_to_point)
  ...
  if(create_mesh == "from_shapefile"){
    ...
    mesh <- inla.mesh.2d(
      ...
      cutoff=0.1/k,crs = grid_projection) ## good to have >0
  ...
  mesh_pred <- fm_rcdt_2d(as.matrix(loc_x[,c('long','lati')] ))
  W <- rep(1,nrow(loc_x))
...
#' @param grid_projection character A character string of projection arguments for `sf`

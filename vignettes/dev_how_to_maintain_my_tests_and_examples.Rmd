---
title: "Dev - How to maintain my tests and-examples.Rmd empty"
...
We have chosen to store the results of the functions in the `examples` folder. This allows the outputs to be reused in the examples with the following code:
...
The unit tests for the "big" model are not yet implemented. Due to the amount of data, only some of it will need to be stored and in the `tests/testthat` folder as this will only be used for testing.

### Updating outputs

As a dev, you will need to update the outputs if the functions or dependent packages change. To do this, in each function, the unit tests allow you to update the outputs. Here is the procedure to follow: 
...
If unit tests don't pass because of differences in results, make sure you have the correct package versions for INLA and TMB. You can find the information in the `DESCRIPTION` file with the lines `Config/FishMap/INLA/version` and `Config/FishMap/TMB/version`.
...
## 
## 
## processing file: /home/rstudio/FishMap/dev/dev_how_to_maintain_my_tests_and_examples.Rmd
```

```
## Error in parse_block(g[-1], g[1], params.src, markdown_mode): Duplicate chunk label 'development', which has been used for the chunk:
## library(testthat)
```

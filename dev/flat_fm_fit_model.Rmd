...
test_that("fm_fit_model works", {
  ...
  if(test_resolution == "small"){
    ...
    fm_data_inputs <- readr::read_rds(
      file.path("data", "part1_output_big.rds")
      )
  }
  ...
  if (Sys.getenv("FISHMAP_UPDATE_TEST_OUTPUTS") == "TRUE") {
    ...
    #' @description Test to check if we can save output big
      expect_true(file.exists(file.path("data", paste0("part2_output_", test_resolution , ".rds"))))
    }
  ...
  if(test_resolution == "small"){
    ...
    expected_outputs <- readr::read_rds(
      file.path("data", paste0("part2_output_", test_resolution , ".rds"))
      )
  }
...
fusen::inflate(flat_file = "dev/flat_fm_fit_model.Rmd", vignette_name = "B - Dev - Fit Model", check = FALSE)
```
